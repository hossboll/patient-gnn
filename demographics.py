            ...
            for repetition in range(self.repetitions):
                ...
                for epoch in range(self.epochs):
                    ...
                    if epoch == self.epochs - 1:
                        torch.save(model, f"/PATH/demographics/{graph_type}-{repetition+1}-f1{val_f1}")
...
if __name__ == "__main__":
    ...
    alpha = 0.75
    gamma = 1
    experiment = DemographicsExperiment(base_model_path, graph_dir, epochs, repetitions, loss_type, alpha, gamma)
    experiment.run_experiment("EXP-NAME")


    #wandb sync --clean for deleting wdb cache
    #'tmux attach -t [session_name]' to view each sessio, ctrlb , d for detaching

    # checkpoint = basemodel architecture + focal loss (alpha = 0.75, gamma = 1) = avid-firefly-85